        ...
        <section class="recommendation-band">
            ...
            <div class="row g-3">
                @foreach($recommendations as $rec)
                    <div class="col-6 col-md-3"><div class="recommendation-card h-100"><a href="{{ route('products.show', $rec->id) }}">@if($rec->image)<img src="{{ asset('storage/' . $rec->image) }}" class="recommendation-image" alt="{{ $rec->name }}">@else<div class="recommendation-placeholder"><i class="bi bi-image text-muted fs-2"></i></div>@endif</a><div class="p-3 d-flex flex-column h-100"><a href="{{ route('products.show', $rec->id) }}" class="text-decoration-none text-dark fw-bold small mb-2">{{ $rec->name }}</a><div class="mt-auto d-flex justify-content-between align-items-center"><span class="text-danger fw-bold small">{{ number_format($rec->price, 0, ',', '.') }} đ</span><a href="{{ route('products.show', $rec->id) }}" class="btn btn-sm btn-outline-primary rounded-circle" title="Xem sản phẩm"><i class="bi bi-arrow-up-right"></i></a></div></div></div></div>
                @endforeach
            </div>
        </section>
    @endif
